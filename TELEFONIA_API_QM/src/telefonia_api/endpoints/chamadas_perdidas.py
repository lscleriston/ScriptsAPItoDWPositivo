"""Endpoint: chamadas perdidas (DetailsDO.CallsLost)
...
# ---------------------------------------------------------------------------
# Helpers (mesma lógica do script antigo, adaptada para CallsLost)
# ---------------------------------------------------------------------------
...
def _extrair_chamadas(api_response, fila, operacao, ano, block_key="DetailsDO.CallsKO"):
...
def process(client_config: Dict[str, Any], client_name: str):
    ...
    nome_tabela = "tb_telefonia_perdidas"
    ...
    for queue in queues:
        for ano, mes in meses:
            ...
            print(f"[INFO] Coletando chamadas perdidas de {queue} para {mes:02d}/{ano} "
            ...
            month_data = []
            responses = client.get_data(queue, from_p, to_p, "DetailsDO.CallsKO")
            ...
            if not month_data:
                print(f"[INFO] Nenhuma chamada perdida em {queue} {mes:02d}/{ano}")
                continue
