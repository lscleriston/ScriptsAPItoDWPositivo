...
class ZabbixClient:
	...
	Usa token de API no cabecalho Authorization: Bearer <token>.
	...
	def get_triggers(self, status: Optional[int] = 0, with_hosts: bool = True, limit: int = 10000) -> List[Dict[str, Any]]:
		"""Recupera triggers (por padrão apenas ativas status=0), incluindo tags."""
		params = {
	...
	def get_acknowledges_for_events(self, eventids: List[str]) -> List[Dict[str, Any]]:
		"""Recupera acknowledges para uma lista de eventos (selectAcknowledges=extend)."""
		if not eventids:
			return []
		params = {
			"output": ["eventid"],
